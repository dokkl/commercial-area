...
**현재 상태: 구현 진행 중.** 데이터 적재 파이프라인과 조회 계층은 동작하지만
HTTP API와 화면은 아직 붙지 않았다. 자세한 내용은 아래 [구현 상태](#구현-상태) 참조.
...
| 구성 요소 | 상태 |
|---|---|
| Docker Compose 환경 (MySQL 8.4 + 앱) | 완료 |
| 스키마 (`store` / `region` / `industry`) | 완료 |
| CSV 파서 | 완료 |
| 배치 적재 · 룩업 테이블 생성 | 완료 |
| 격자 집계 쿼리 | 완료 |
| 점 · 목록 · 상세 조회 쿼리 | 완료 |
| 표시 모드 결정 · 오류 응답 규약 | 완료 |
| `GET /api/map` | 작업 중 |
| `GET /api/stores`, `GET /api/stores/{id}` | 예정 |
| 지역 · 업종 룩업 API | 예정 |
| 지도 화면 | 예정 |
| 필터 패널 · 결과 목록 | 예정 |

**따라서 지금 `http://localhost:8080`을 열면 404가 뜬다.** 화면은 아직 없다.
현재 검증 가능한 것은 데이터 적재와 그 결과다 (위 [적재 결과 확인](#적재-결과-확인) 참조).
