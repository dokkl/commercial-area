...
상권 추이 기능은 분기별 스냅샷을 누적해야 동작한다. 새 분기 CSV를 받아 `.env`의 `CSV_DIR`을
...
- 상권 추이 — "상권 추이 보기"로 선택한 지역·업종의 분기별 점포수 추이와 개·폐업(신규/폐업)
  건수. 분기별 스냅샷을 누적 적재해야 동작한다.
...
- 인증·권한이 없다. 로컬 전용이다.
- 클라우드 배포 설정이 없다.
- 적재는 스냅샷(`SNAPSHOT`) 단위로 멱등하다. `import_log`에 완료 기록이 있으면 건너뛰고,
  중단된 스냅샷은 다음 기동에서 그 스냅샷을 처음부터 다시 적재한다(부분 상태로 남지 않는다).
