...
| 가정 | 예상 | 빗나갈 경우 대응 |
|---|---|---|
| 122만 행 배치 적재 소요 | 2~4분 | 배치 크기 조정, `LOAD DATA` 재검토 |
| 서울 전역 줌아웃 격자 집계 응답 | 1초 이내 | 줌 레벨별 격자 키를 생성 컬럼(STORED)으로 두고 인덱싱 |
| 점 모드 상한 2,000건 렌더링 | 부드러움 | 상한을 1,000으로 낮춤 |
| 필터 + bbox 복합 인덱스 활용 | 인덱스 스캔 | `EXPLAIN` 확인 후 인덱스 순서 조정 |

특히 두 번째 항목이 가장 불확실하다. 서울 전역을 줌아웃해서 보면 55만 행을 집계해야
하므로, 여기서 느리면 격자 키 컬럼 도입이 필요하다. 이는 스키마에 컬럼과 인덱스를
추가하는 국소 변경이라 설계 전체에 영향을 주지 않는다.
